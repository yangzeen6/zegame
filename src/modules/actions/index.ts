...
export class ActionManager {
    ...
    async match(session: ZeSessionBase) {
        ...
        if (!this.block[session.event.sender_id] && (!session.event.group_id || await this.core.System.group_list_check(session.event.group_id))) {
            ...
            if (action) {
                if (action.rule) {
                    const rls = Array.isArray(action.rule) ? action.rule : [action.rule]
                    for (const rl of rls) {
                        if(!await rl(this.core, session))
                            return;
                    } 
                }
                this.block[session.event.sender_id] = true;
                await action.action(this.core, session, args);
                delete this.block[session.event.sender_id];
            }
        }
    }
}
